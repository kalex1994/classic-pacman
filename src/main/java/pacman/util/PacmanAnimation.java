...
public class PacmanAnimation {
	...
	/**
	 * Constructor for creating a {@code PacmanAnimation} object. Creates
	 * rotations of the images upwards, downwards and to the left.
	 * 
	 * @param images
	 *            images for the animation, all image faces to the right
	 * @param order
	 *            order of the images to be drawn
	 */
	...
	/**
	 * Creates rotations of the images in the source array and places them in
	 * the array specified. The rotated images will face to the direction
	 * specified.
	 * 
	 * @param source
	 *            the source array of images, all image faces to the right
	 * @param destination
	 *            array where the rotated images will be placed
	 * @param direction
	 *            after the rotation, all image will face to the direction
	 *            specified
	 */
	void createRotation(Image source[], Image destination[], Direction direction) {
		for (int i = 0; i < source.length; ++i) {
			...
			BufferedImage copyOfImage = new BufferedImage(width + 1,
	 ...
	 */
	public void draw(Graphics2D g, Pacman pacman) {
		...
		g.drawImage(current[order[index]], pacman.position.x - 3,
				pacman.position.y - 3, null);
	}
