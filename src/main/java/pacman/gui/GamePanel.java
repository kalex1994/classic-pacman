...
/**
 * Custom {@link JPanel} where the game takes place on. All player interaction processed
 * here.
 */
...
public class GamePanel extends JPanel implements ActionListener {
	...
	/**
	 * Timer for udpating the game's state. In every timer tick the game's state
	 * gets updated.
	 */
	private Timer timer;
	...
	/**
	 * Initializes a new level. Called at the start of the game, and after every
	 * completed level.
	 */
	...
	/**
	 * Initializes the level after pacman died. The ghosts and Pacman are
	 * returning to their initial cells. The player's score reamains unchanged.
	 */
	...
	/**
	 * If Pacman reaches a cell that contains food, the player's score is
	 * updated. If all food has been eaten, a new level starts.
	 */
	...
	/**
	 * Called when Pacman has no more lives. The player's high score is written
	 * to the database.
	 */
	...
	/**
	 * Draws a ghost. A ghost has 4 different images according to what direction
	 * it is moving.
	 * 
	 * @param g
	 *            {@link java.awt.Graphics Graphics} object used for drawing
	...
	private void drawGhost(Graphics g, Ghost ghost) {
		...
		if (ghost.getName().equals("pinky"))
			g.drawImage(images.pinky[ghost.getDirection().ordinal()],
					ghost.position.x - 3, ghost.position.y - 3, null);
		else if (ghost.getName().equals("blinky"))
			g.drawImage(images.blinky[ghost.getDirection().ordinal()],
					ghost.position.x - 3, ghost.position.y - 3, null);
		else if (ghost.getName().equals("inky"))
			g.drawImage(images.inky[ghost.getDirection().ordinal()],
					ghost.position.x - 3, ghost.position.y - 3, null);
		else if (ghost.getName().equals("clyde"))
			g.drawImage(images.clyde[ghost.getDirection().ordinal()],
					ghost.position.x - 3, ghost.position.y - 3, null);
		else if (ghost.getName().equals("orson"))
			g.drawImage(images.orson[ghost.getDirection().ordinal()],
					ghost.position.x - 3, ghost.position.y - 3, null);
		else
			g.drawImage(images.spooky[ghost.getDirection().ordinal()],
					ghost.position.x - 3, ghost.position.y - 3, null);
	}

	/**
	 * For every cell in the maze, draws a little rectangle at the center of
	 * that cell, if the cell contains food.
	 * 
	 * @param g
	 *            {@link java.awt.Graphics Graphics} object used for drawing
	 */
	...
	/**
	 * Draws level information at the top of the screen.
	 * 
	 * @param g
	 *            {@link java.awt.Graphics Graphics} object used for drawing
	 */
	...
	/**
	 * Custom {@link KeyAdapter} used for processing user input.
	 */
