...
/**
 * Class representing a ghost in the game. If a ghost reaches Pacman, Pacman
 * dies. The ghost moves randomly in the maze. It selects a random empty cell,
 * then it moves toward that cell on the shortest path. Each ghost spawns on a
 * unique maze position.
 */
public class Ghost {
	/**
	...
	/**
	 * A {@link pacman.util.ShortestPath ShortestPath} object for getting the
	 * shortest path from a source cell to a destination cell.
	 */
	private ShortestPath shortestPath = new ShortestPath();

	/**
	 * List of directions to move along the shortest path from a source cell to
	 * a destination cell.
	 */
	...
	/**
	 * Constructor for creating a {@code Ghost} object. A target cell is
	 * selected randomly for the ghost. The shortest path to the target cell is
	 * computed then.
	 * 
	 * @param maze
	 *            the maze where the game takes place on
	 ...
	 *            the name of the ghost
	 * @param position
	 *            the (x, y) coordinates of the ghost in the maze
	 */
	public Ghost(Maze maze, String name, Point position) {
		...
		while (true) {
			...
			logger.info("{} has new target cell: {} {}", name,
					targetCell.getRow(), targetCell.getColumn());
			if (!currentCell.equals(targetCell))
				break;
		}

		this.path = shortestPath.breadthFirstSearch(maze, currentCell,
				targetCell);
	...
	/**
	 * Updates the ghost's position according to its direction. If the ghost
	 * reaches a new cell, its current cell is updated, and gets a new randomly
	 * chosen target cell. The shortest path from the current cell to the target
	 * cell is computed then.
	 */
	public void update() {
		if (position.x % 15 == 0 && position.y % 15 == 0) {
			...
			if (currentCell.equals(targetCell)) {
				...
				path = shortestPath.breadthFirstSearch(maze, currentCell,
						targetCell);
			}
	...
	/**
	 * Returns whether the ghost collided with {@link Pacman}.
	 * 
	 * @param pacman
	 *            represents Pacman in the game
	 * @return whether the ghost collided with {@link Pacman}
	 */
	public boolean isCollidedWithPacman(Pacman pacman) {
		if (Math.hypot(position.x - pacman.getPosition().x,
				position.y - pacman.getPosition().y) < 10.0)
			return true;
		else
			return false;
	}
